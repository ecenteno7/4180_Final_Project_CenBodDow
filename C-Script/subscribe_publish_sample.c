...
int main(int argc, char **argv) {
	...
	while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
		...
		//printf("%s\n", strtok(msg, "_"));
		//alarm_time = msg;
		if(msg != NULL && msg[0] == '2' && executed == 0){
			executed = 1;
			is_alarm = 1;
			strtok(msg, "_");
			day = atoi(strtok(NULL, "_"));
			month = atoi(strtok(NULL, "_"));
			year = atoi(strtok(NULL, "_"));
			hour = atoi(strtok(NULL, "_"));
			minute = atoi(strtok(NULL, "_"));
			char guiCode[12] = "1_A_";
			paramsQOS0.payload = (void*)strtok(NULL, "_");
			strcat(guiCode, paramsQOS0.payload);
			paramsQOS0.payload = guiCode;
			paramsQOS0.payloadLen = strlen(paramsQOS0.payload);
			...
			printf("%d\n", time_info->tm_min);
			printf("%d\n", time_info->tm_sec);*/
		} else if(msg != NULL && msg[0] == '3' && executed == 0) {
			executed = 1;
			strtok(msg, "_");
			alarm_check = atoi(strtok(NULL, "_"));
			if (alarm_check == 0){
				//printf("Alarm OFF\n");
				system("pkill -f aplay");
				system("sudo pkill -f python3");
				system("sudo python3 /home/pi/Desktop/led_ring/led_ring_8.py");
				system("sudo pkill -f python3");
				curr_led=0;
				sprintf(led_scripts, "sudo python3 /home/pi/Desktop/led_ring/led_ring_%d.py &",curr_led);
			} else if(alarm_check == 1) {
				//printf("Alarm UP\n");
				system("amixer sset PCM 5%+");
				curr_led = (curr_led + 1 < 7) ? curr_led + 1 : 7;
				system("sudo pkill -f python3");
				sprintf(led_scripts, "sudo python3 /home/pi/Desktop/led_ring/led_ring_%d.py &", curr_led);
				system(&led_scripts);
			}
		...
		if(is_alarm == 1 && year == time_info->tm_year + 1900) {
			if(month == time_info->tm_mon + 1) {
				if(day == time_info->tm_mday) {
					if(hour == time_info->tm_hour) {
						if(minute == time_info->tm_min) {
							is_alarm = 0;
							system("amixer sset 'PCM' 70%");
							system("aplay /home/pi/Downloads/star-wars-theme-song.wav &");
							system(&led_scripts);
